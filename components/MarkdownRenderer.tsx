...
function splitMermaid(content: string): Segment[] {
  ...
  while ((match = fence.exec(content)) !== null) {
    if (match.index > cursor) {
      segments.push({ type: 'markdown', text: content.slice(cursor, match.index) })
    }
  ...
  return segments.filter(s => s.type === 'mermaid' ? s.chart.trim() : s.type === 'markdown' && s.text.trim())
}
...
function extractTrailingTsBlock(text: string): { before: string; code: string } | null {
...
const MD_COMPONENTS: React.ComponentProps<typeof ReactMarkdown>['components'] = {
  ...
  code: ({ className, children, ...props }) => {
    ...
    return <code className="dfh-code-inline" {...props}>{children}</code>
  },
  blockquote: ({ children }) => <blockquote className="dfh-blockquote">{children}</blockquote>,
  h1: ({ children }) => <h1 id={headingId(children)} className="dfh-h1 scroll-mt-24">{children}</h1>,
  h2: ({ children }) => <h2 id={headingId(children)} className="dfh-h2 scroll-mt-24">{children}</h2>,
  h3: ({ children }) => <h3 id={headingId(children)} className="dfh-h3 scroll-mt-24">{children}</h3>,
  h4: ({ children }) => <h4 id={headingId(children)} className="dfh-h4 scroll-mt-24">{children}</h4>,
  ...
  strong: ({ children }) => <strong className="dfh-strong">{children}</strong>,
