    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 4, marginLeft: 'auto', alignItems: 'center' }}>
          <span style={{
            fontFamily: 'ui-monospace, monospace',
            fontSize: '0.65rem', color: 'var(--fg-gutter)', minWidth: 36, textAlign: 'right',
          }}>
            {Math.round(scale * 100)}%
          </span>
          <button style={BTN} onClick={() => { setActive(true); setScale(s => Math.min(10, s * 1.25)) }} title="Zoom in">+</button>
          <button style={BTN} onClick={() => { setActive(true); setScale(s => Math.max(0.05, s * 0.8)) }} title="Zoom out">−</button>
          <button style={BTN} onClick={fitToContainer} title="Fit to view" aria-label="Reset zoom">
      ...
      >
        ...
        {error && (
          <pre style={{ color: 'var(--red)', fontSize: '0.8rem', padding: '1rem', margin: 0 }}>
            [Mermaid parse error]{'\n'}{error}
          </pre>
        )}
      </div>
