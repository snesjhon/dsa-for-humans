...
```
Start:         freq = {}
See 'cat':     freq.get('cat') → undefined. Set 'cat' → 1.   freq = {cat:1}
See 'dog':     freq.get('dog') → undefined. Set 'dog' → 1.   freq = {cat:1, dog:1}
See 'cat':     freq.get('cat') → 1.         Set 'cat' → 2.   freq = {cat:2, dog:1}
See 'bird':    freq.get('bird') → undefined. Set 'bird' → 1. freq = {cat:2, dog:1, bird:1}
See 'dog':     freq.get('dog') → 1.         Set 'dog' → 2.   freq = {cat:2, dog:2, bird:1}
See 'cat':     freq.get('cat') → 2.         Set 'cat' → 3.   freq = {cat:3, dog:2, bird:1}
```
...
```typescript
// Build a character frequency table for a string
...
```
Start:    seen = {}
See 3:    seen.has(3)? No. Add.   seen = {3}
See 1:    seen.has(1)? No. Add.   seen = {3, 1}
See 4:    seen.has(4)? No. Add.   seen = {3, 1, 4}
See 1:    seen.has(1)? YES → return true immediately.
```
...
// allDistinct([1, 2, 3]) → true
// allDistinct([1, 2, 1]) → false
function allDistinct(nums: number[]): boolean {
...
**How to think about it**
Maintain a running `prefixSum` as you walk the array. You want to know: does any subarray ending *here* have sum exactly `k`? If the running sum at position `j` is `S`, and there was a running sum of `S − k` at some earlier position `i`, then the slice from `i+1` to `j` sums to exactly `k`. Store all seen prefix sums in a map (prefixSum → count of how many times that sum appeared) and check `map.has(prefixSum − k)` at each step.
...
```
Initialize: prefixSum = 0, counts = {0:1}, result = 0

Index 0, value 1:
  prefixSum = 0 + 1 = 1
  Look for 1 − 2 = −1 in counts → not found, add 0
  counts.set(1, 1)        counts = {0:1, 1:1}

Index 1, value 1:
  prefixSum = 1 + 1 = 2
  Look for 2 − 2 = 0 in counts → found (value: 1), result += 1 → result = 1
  counts.set(2, 1)        counts = {0:1, 1:1, 2:1}

Index 2, value 1:
  prefixSum = 2 + 1 = 3
  Look for 3 − 2 = 1 in counts → found (value: 1), result += 1 → result = 2
  counts.set(3, 1)        counts = {0:1, 1:1, 2:1, 3:1}
```
...
function subarrayCount(nums: number[], k: number): number {
  ...
  counts.set(0, 1); // seed: the empty prefix has sum 0
...
```typescript
// Find two indices whose values sum to target (exactly one solution guaranteed)
// twoSum([2,7,11,15], 9)  → [0,1]
// twoSum([3,2,4],    6)   → [1,2]
function twoSum(nums: number[], target: number): number[] {
  const catalog = new Map<number, number>(); // value → index
  for (let i = 0; i < nums.length; i++) {
    const complement = target - nums[i];
    if (catalog.has(complement)) return [catalog.get(complement)!, i];
    catalog.set(nums[i], i); // store after checking — prevents self-match
  }
  return [];
}
```
