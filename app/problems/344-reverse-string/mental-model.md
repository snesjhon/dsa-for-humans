...
Row of cards: 1, 4, 3, 2, 6, 5

- Front dealer at card **1** (far left), back dealer at card **5** (far right). Swap. Row becomes: **5**, 4, 3, 2, 6, **1**. Both step inward.
- Front dealer at card **4** (position 1), back dealer at card **6** (position 4). Swap. Row becomes: 5, **6**, 3, 2, **4**, 1. Both step inward.
- Front dealer at card **3** (position 2), back dealer at card **2** (position 3). Swap. Row becomes: 5, 6, **2**, **3**, 4, 1. Both step inward.
- Front dealer now at position 3, back dealer at position 2. The back dealer is to the *left* of the front dealer — they've crossed. Stop.

The row is now 5, 6, 2, 3, 4, 1 — the exact reverse of 1, 4, 3, 2, 6, 5.
...
Tracing `[1, 4, 3, 2, 6, 5]`:
- Setup: frontDealer=0, backDealer=5 → arr = [1, 4, 3, 2, 6, 5]
- Swap arr[0]=1 with arr[5]=5, frontDealer=1, backDealer=4 → arr = [5, 4, 3, 2, 6, 1]
- Swap arr[1]=4 with arr[4]=6, frontDealer=2, backDealer=3 → arr = [5, 6, 3, 2, 4, 1]
- Swap arr[2]=3 with arr[3]=2, frontDealer=3, backDealer=2 → arr = [5, 6, 2, 3, 4, 1]
- frontDealer(3) >= backDealer(2): stop
- Output: [5, 6, 2, 3, 4, 1] ✓
...
function reverseArray(arr: number[]): void {
  // station the front dealer at the first card
  // station the back dealer at the last card
  // (the march and swap come in step 2)
}
```
...
```mermaid
graph TD
    A["Start: front=0, back=5<br/>arr = [1, 4, 3, 2, 6, 5]"]
    B["Round 1: swap index 0 and 5<br/>front=1, back=4<br/>arr = [5, 4, 3, 2, 6, 1]"]
    C["Round 2: swap index 1 and 4<br/>front=2, back=3<br/>arr = [5, 6, 3, 2, 4, 1]"]
    D["Round 3: swap index 2 and 3<br/>front=3, back=2<br/>arr = [5, 6, 2, 3, 4, 1]"]
    E["front(3) >= back(2) — dealers crossed, done!"]
    A --> B
    B --> C
    C --> D
    D --> E
...
// SKETCH ONLY — no working implementation here
function reverseArray(arr: number[]): void {
  // dealers start here (from step 1)
  while (/* dealers haven't met or crossed */) {
    // front dealer grips their card briefly (heldCard)
    // both dealers exchange what they're holding
    // each dealer steps one position inward
  }
}
```
