...
export default function ProblemPage({ params }: Props) {
  ...
  if (primarySection) {
    const allInSection = [...primarySection.firstPass, ...primarySection.reinforce]
  ...
  return (
    <div className="block lg:grid w-full items-start" style={{ gridTemplateColumns: '260px 1fr 260px', columnGap: '6rem' }}>
      ...
      <aside className="hidden lg:block sticky top-20 self-start max-h-[calc(100vh-5rem)] overflow-y-auto">
        <TableOfContents headings={headings} title="Mental Model" />
      </aside>
      ...
      <article className="min-w-0 space-y-8 py-2">
        ...
        <div className="flex items-center gap-2 text-sm" style={{ color: 'var(--fg-gutter)' }}>
          ...
          {primarySection && <><span>/</span><span style={{ color: 'var(--fg-comment)' }}>{primarySection.label}</span></>}
          <span>/</span>
        ...
        {/* Header */}
        <div>
          <p className="text-xs font-mono mb-2" style={{ color: 'var(--fg-gutter)' }}>#{problem.id}</p>
        ...
        {mentalModelContent ? (
          <MarkdownRenderer content={mentalModelContent} problemSlug={problem.slug} />
        ) : (
