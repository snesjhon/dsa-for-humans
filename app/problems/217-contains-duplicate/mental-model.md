...
Say the pile holds four stamps with designs `1`, `2`, `3`, `1`.

The collector opens an empty album. First stamp: design `1`. Not in album — mount it. Second stamp: design `2`. Not in album — mount it. Third stamp: design `3`. Not in album — mount it. Fourth stamp: design `1`. Check album — design `1` is already mounted! Duplicate confirmed. Stop immediately: the answer is `true`.
...
Tracing `[1, 2, 3, 1]`:
- Setup: `stampAlbum = {}` (empty), begin scanning the pile
- Stamp `1`: not in album → add; album is now `{1}`
- Stamp `2`: not in album → add; album is now `{1, 2}`
- Stamp `3`: not in album → add; album is now `{1, 2, 3}`
...
function containsDuplicate(nums: number[]): boolean {
  // open the album — no designs recorded yet
  // return false by default: no duplicates seen yet
}
```
...
for (const stamp of nums) {
  if (/* stamp is already in album */) return true;
  // add stamp to album for future checks
}
return false;
```
